    ...
    if (resize_pending) {
        if (new_w == s->draw.width && new_h == s->draw.height) {
            ...
            if (!new_framebuf || !new_prev_framebuf || !new_send_buf0 || !new_send_buf1) {
                ...
                /* Resize wlroots output to LOGICAL dimensions.
                 * The compositor renders at logical resolution.
                 * 9front's 'a' command scales to physical window.
                 * Do NOT use wlr_output_state_set_scale() - we handle scaling ourselves.
                 */
...
void new_output(struct wl_listener *l, void *d) {
    ...
    /* Compute logical dimensions for the compositor.
     * Physical dimensions come from 9front window (s->width, s->height at init).
     * Logical = physical / scale.
     * Compositor renders at logical resolution; 9front scales to physical via 'a' command.
     */
    float scale = s->scale;
    if (scale <= 0.0f) scale = 1.0f;
    ...
    /* If scaling, reallocate buffers at logical resolution */
    if (scale > 1.0f && (logical_w != phys_w || logical_h != phys_h)) {
        size_t fb_size = logical_w * logical_h * sizeof(uint32_t);
        uint32_t *new_framebuf = calloc(1, fb_size);
        uint32_t *new_prev_framebuf = calloc(1, fb_size);
        uint32_t *new_send_buf0 = calloc(1, fb_size);
    ...
    /* Update draw state - physical dimensions stay as-is, add logical */
    ...
    if (scale > 1.0f) {
        ...
        wlr_log(WLR_INFO, "Output ready: %dx%d", logical_w, logical_h);
    }
}
